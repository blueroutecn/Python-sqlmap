...
class Connect(object):
    ...
    def getPage(**kwargs):
        ...
        processResponse(page, responseHeaders) #processResponse会调用到./lib/parse/html.py中的htmlParser函数 函数就是根据不同的数据库指纹去识别当前的数据库究竟是什么
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None, timeBasedCompare=False, noteResponseTime=True, auxHeaders=None, response=False, raise404=None, removeReflection=True):
        ...
        if timeBasedCompare:
            return wasLastResponseDelayed() #函数wasLastRequestDelayed()的功能主要是判断最后一次的请求是否有明显的延时
        elif noteResponseTime: #当timeBasedCompare为True（即进行time-based blind注入检测）时，直接返回执行结果，如果是其他类型的请求，就保存响应时间。
