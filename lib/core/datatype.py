...
class AttribDict(dict):
    ...
    type dictionary. 这个类定义了sqlmap对象,继承从Python数据类型字典。
...
class InjectionDict(AttribDict):
    def __init__(self):
        ...
        # all the information specific for that stype 数据是dict各种药布,每个是dict药栓与特定的所有信息
        self.data = AttribDict()

        # conf is a dict which stores current snapshot of important
        # options used during detection 配置是一个商店的当前快照的dict重要选项用于检测
