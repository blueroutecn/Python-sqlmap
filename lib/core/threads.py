...
def readInput(message, default=None):
    # It will be overwritten by original from lib.core.common 它从lib.core.common将由原来的覆盖
    pass

def getCurrentThreadData():
    """
    Returns current thread's local data 返回当前线程的本地数据
    """
...
def getCurrentThreadName():
    """
    Returns current's thread name 返回当前的线程的名称
    """
...
def runThreads(numThreads, threadFunction, cleanupFunction=None, forwardException=True, threadChoice=False, startThreadMsg=True):
    ...
    if threadChoice and numThreads == 1 and any(_ in kb.injection.data for _ in (PAYLOAD.TECHNIQUE.BOOLEAN, PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.QUERY, PAYLOAD.TECHNIQUE.UNION)):
        ...
        if numThreads == 1:
            warnMsg = "running in a single-thread mode. This could take a while." #在一个单线程模式下运行。这可能需要一段时间
            logger.warn(warnMsg)
