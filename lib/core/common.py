...
def readInput(message, default=None, checkBatch=True):
    """
    Reads input from terminal 从终端读取输入
    """
    ...
    if retVal is None:
        if checkBatch and conf.batch:
            ...
            debugMsg = "used the default behaviour, running in batch mode"#使用默认的行为,在批处理模式下运行
            logger.debug(debugMsg)
...
def randomRange(start=0, stop=1000):
    """
    Returns random integer value in given range 随机整数返回值在一定范围内

    >>> random.seed(0)
    >>> randomRange(1, 500) 1-500之间
    423
    """
...
def checkFile(filename):
    """
    Checks for file existence 检查文件是否存在
    """

    if not os.path.isfile(filename):
        raise SqlmapFilePathException("unable to read file '%s'" % filename)#无法读取文件

def banner():
    """
    This function prints sqlmap banner with its version 这个函数打印sqlmap旗帜的版本
    """
...
def setPaths():#注释 项目集目录和文件的绝对路径
    """
    Sets absolute paths for project directories and files
    """

    # sqlmap paths 定义路径和文件
...
def parseTargetUrl(): #parseTargetUrl()函数主要完成针对目标网址的解析工作，如获取协议名、路径、端口、请求参数等信息
    """
    Parse target URL and set some attributes into the configuration singleton.目标URL和设置一些属性解析成单的配置
    """
...
def parseFilePaths(page):
    """
    Detects (possible) absolute system paths inside the provided page content 检测系统(可能)绝对路径里面所提供的页面内容
    """
...
def wasLastResponseDBMSError():
    """
    Returns True if the last web request resulted in a (recognized) DBMS error page 返回True,如果最后一个web请求导致DBMS错误页面(认可)
    """
...
def wasLastResponseHTTPError():
    """
    Returns True if the last web request resulted in an errornous HTTP code (like 500) 返回True,如果最后一个web请求导致了errornous HTTP代码(如500
    """

    threadData = getCurrentThreadData()
    return threadData.lastHTTPError and threadData.lastHTTPError[0] == threadData.lastRequestUID

def wasLastResponseDelayed(): #函数wasLastRequestDelayed()的功能主要是判断最后一次的请求是否有明显的延时
    """
    Returns True if the last web request resulted in a time-delay 返回True,如果最后一个web请求导致延时
    """
    #将最后一次请求的响应时间与之前所有请求的响应时间的平均值进行比较，如果最后一次请求的响应时间明显大于之前几次请求的响应时间的平均值，就说明有延迟
    ...
    if deviation and not conf.direct:
        if len(kb.responseTimes) < MIN_TIME_RESPONSES:
            ...
            warnMsg += "with less than %d response times" % MIN_TIME_RESPONSES #基于时间的标准偏差方法与不到**模型响应时间
            logger.warn(warnMsg)
...
def urlencode(value, safe="%&=", convall=False, limit=False, spaceplus=False):
    """
    URL encodes given value #URL编码给定值
    ...
    if value:
        ...
        # except in cases when tampering scripts are used 角情况下字符%真的需要编码(当不代表URL编码字符)除了在使用情况下篡改脚本
...
def extractRegexResult(regex, content, flags=0):
    ...
    content 返回“结果”组值从一个给定的内容可能与正则表达式相匹配
    #主要使用正则表达式判断是否包含指定的数据，如果有，则返回匹配的数据，没有，则返回None 如果url可以注入的话，返回值retVal应该等于"1"
...
def getSortedInjectionTests():
    ...
    messages 最终发现DBMS返回优先测试列表的错误消息
    """
...
def hashDBRetrieve(key, unserialize=False, checkConf=False):
    """
    Helper function for restoring session data from HashDB  从HashDB Helper函数恢复会话数据
