...
def _setRequestParams():
    ...
    HTTP method POST. 检查和设置参数和执行检查在“数据”选项 HTTP POST方法
    """
    ...
    # Perform checks on GET parameters ִ    执行检查参数GET
    ...
    # Perform checks on POST parameters 对POST参数执行检查
    ...
    if conf.data is not None:
        ...
        if not kb.postHint:
            ...
            if CUSTOM_INJECTION_MARK_CHAR not in conf.data:  # in case that no usable parameter values has been found 若没有可用的参数被发现
...
def initTargetEnv(): #initTargetEnv()函数主要就是完成全局变量conf和kb的初始化工作
    """
    Initialize target environment.初始化目标环境
    """
...
def setupTargetEnv():
    _createTargetDirs() #创建输出结果目录
    _setRequestParams() #解析请求参数
    _setHashDB() #设置session信息，就是session.sqlite
    _resumeHashDBValues() # 恢复session的数据，继续扫描
    _setResultsFile() #存储扫描结果
    _setAuthCred() #添加认证信息
