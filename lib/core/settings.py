...
# Payload used for checking of existence of IDS/WAF (dummier the better) #payload用于检查存在的id / WAF(假更好)
IDS_WAF_CHECK_PAYLOAD = "AND 1=1 UNION ALL SELECT 1,2,3,table_name FROM information_schema.tables WHERE 2>1"

# Vectors used for provoking specific WAF/IDS/IPS behavior(s) 向量用于引发特定WAF / IDS / IPS行为(s)
WAF_ATTACK_VECTORS = (
                        "",  # NIL
...
ROTATING_CHARS = ('\\', '|', '|', '/', '-') #用于状态表示字典攻击阶段

# Chunk length (in items) used by BigArray objects (only last chunk and cached one are held in memory)
BIGARRAY_CHUNK_LENGTH = 4096 #块长度BigArray对象(项目)使用的缓存(只有最后一块和一个在内存)

# Only console display last n table rows
TRIM_STDOUT_DUMP_SIZE = 256 #只有控制台显示去年n表行

# Parse response headers only first couple of times
PARSE_HEADERS_LIMIT = 3 #解析响应头只有第一个几次

# Step used in ORDER BY technique used for finding the right number of columns in UNION query injections
ORDER_BY_STEP = 10 #步骤按顺序使用技术用于寻找合适的注射联合查询中的列数

# Maximum number of times for revalidation of a character in time-based injections
MAX_TIME_REVALIDATION_STEPS = 5 #最大数量的时间重新生效的一个角色在基于时间的注射

# Characters that can be used to split parameter values in provided command line (e.g. in --tamper)
PARAMETER_SPLITTING_REGEX = r'[,|;]' #字符,可以用来分割在提供了命令行参数值(例如,--tamper)

# Regular expression describing possible union char value (e.g. used in --union-char)
UNION_CHAR_REGEX = r'\A\w+\Z' #正则表达式描述可能的联盟字符值(例如用于--union-char)

# Attribute used for storing original parameter value in special cases (e.g. POST)
UNENCODED_ORIGINAL_VALUE = 'original' #属性用于存储原始参数值在特殊情况下(例如post)

# Common column names containing usernames (used for hash cracking in some cases) 常见的列名包含用户名(用于hash在某些情况下)
...
# Default delimiter in GET/POST values 默认的分隔符在GET / POST值
DEFAULT_GET_POST_DELIMITER = '&'

# Default delimiter in cookie values 默认的分隔符在cookie的值
DEFAULT_COOKIE_DELIMITER = ';'

# Unix timestamp used for forcing cookie expiration when provided with --load-cookies
FORCE_COOKIE_EXPIRATION_TIME = "9999999999" #Unix时间戳用于迫使cookie过期当提供——load-cookies

# Skip unforced HashDB flush requests below the threshold number of cached items
HASHDB_FLUSH_THRESHOLD = 32 #跳过非受迫性HashDB刷新请求低于阈值缓存条目的数量

# Number of retries for unsuccessful HashDB flush attempts
HASHDB_FLUSH_RETRIES = 3 #重试次数HashDB冲洗尝试失败

# Unique milestone value used for forced deprecation of old HashDB values (e.g. when changing hash/pickle mechanism) 独特的里程碑值用于被迫弃用旧HashDB值改变时(如散列/泡菜机制)
HASHDB_MILESTONE_VALUE = "cAWxkLYCQT"  # r5129 "".join(random.sample(string.ascii_letters, 10)) #r5129 " . join(random.sample(字符串。ascii_letters 10))
 
# Warn user of possible delay due to large page dump in full UNION query injections
LARGE_OUTPUT_THRESHOLD = 1024 ** 2 #警告用户可能的延迟由于大页面转储全部注射联合查询
...
DNS_BOUNDARIES_ALPHABET = re.sub("[a-fA-F]", "", string.ascii_letters)#字母用于字符串的前缀和后缀名称解析请求DNS技术(不含十六进制字符不混合与内在内容)

# Alphabet used for heuristic checks 
HEURISTIC_CHECK_ALPHABET = ('"', '\'', ')', '(', '[', ']', ',', '.') #字母用于启发式检查
...
MAX_CONNECTION_CHUNK_SIZE = 10 * 1024 * 1024 # 连接块大小(处理大型反应块以避免MemoryError崩溃——例如大表转储全部联盟注射)

# Maximum response total page size (trimmed if larger)最大响应总页面大小(如果更大的削减)
MAX_CONNECTION_TOTAL_SIZE = 100 * 1024 * 1024

# Maximum (multi-threaded) length of entry in bisection algorithm 最大的长度(多线程)进入二分算法
MAX_BISECTION_LENGTH = 50 * 1024 * 1024

# Mark used for trimming unnecessary content in large chunks
LARGE_CHUNK_TRIM_MARKER = "__TRIMMED_CONTENT__" #马克在大量用于削减不必要的内容

# Generic SQL comment formation 通用SQL注释形成
GENERIC_SQL_COMMENT = "-- "

# Threshold value for turning back on time auto-adjustment mechanism
VALID_TIME_CHARS_RUN_THRESHOLD = 100 #阈值将按时回auto-adjustment机制

# Check for empty columns only if table is sufficiently large
CHECK_ZERO_COLUMNS_THRESHOLD = 10 #检查空表只有列是足够大的

# Boldify all logger messages containing these "patterns" #Boldify所有日志消息包含这些“模式”
...
# Generic www root directory names 通用的www根目录的名称
GENERIC_DOC_ROOT_DIRECTORY_NAMES = ("htdocs", "httpdocs", "public", "wwwroot", "www")

# Maximum length of a help part containing switch/option name(s)
MAX_HELP_OPTION_LENGTH = 18 #最大长度的帮助部分包含开关/选项名称(s)

# Maximum number of connection retries (to prevent problems with recursion)
MAX_CONNECT_RETRIES = 100 #最大数量的连接重试(为了防止递归问题)

# Strings for detecting formatting errors 检测字符串格式错误
...
# Regular expression used for extracting ASP.NET view state values #正则表达式用于提取ASP。网络视图状态值
VIEWSTATE_REGEX = r'(?i)(?P<name>__VIEWSTATE[^"]*)[^>]+value="(?P<result>[^"]+)'

# Regular expression used for extracting ASP.NET event validation values #正则表达式用于提取ASP。事件验证净值
...
LIMITED_ROWS_TEST_NUMBER = 15 #全联盟内的行数来生成测试产量有限(不能太大,以防止负载长度问题)

# Format used for representing invalid unicode characters
INVALID_UNICODE_CHAR_FORMAT = r"\?%02x" #格式用于代表无效的unicode字符

# Regular expression for SOAP-like POST data #正则表达式SOAP-like POST数据
SOAP_RECOGNITION_REGEX = r"(?s)\A(<\?xml[^>]+>)?\s*<([^> ]+)( [^>]+)?>.+</\2.*>\s*\Z"

# Regular expression used for detecting JSON-like POST data 正则表达式用于检测类json POST数据
JSON_RECOGNITION_REGEX = r'(?s)\A(\s*\[)*\s*\{.*"[^"]+"\s*:\s*("[^"]+"|\d+).*\}\s*(\]\s*)*\Z'

# Regular expression used for detecting multipart POST data 正则表达式用于检测弓形POST数据
MULTIPART_RECOGNITION_REGEX = r"(?i)Content-Disposition:[^;]+;\s*name="

# Default POST data content-type 默认的POST数据内容类型
DEFAULT_CONTENT_TYPE = "application/x-www-form-urlencoded; charset=utf-8"

# Raw text POST data content-type 原始文本POST数据内容类型
...
SUHOSIN_MAX_VALUE_LENGTH = 512 #长度,同时检查用于存在Suhosin-patch(像)保护机制

# Minimum size of an (binary) entry before it can be considered for dumping to disk
MIN_BINARY_DISK_DUMP_SIZE = 100 #最小尺寸的(二进制)入口才能被认为是倾销到磁盘

# Regular expression used for extracting form tags 正则表达式用于提取表单标签
FORM_SEARCH_REGEX = r"(?si)<form(?!.+<form).+?</form>"

# Minimum field entry length needed for encoded content (hex, base64,...) check
MIN_ENCODED_LEN_CHECK = 5 #所需的最小字段条目长度编码内容(十六进制,base64,…)检查

# Timeout in seconds in which Metasploit remote session has to be initialized
METASPLOIT_SESSION_TIMEOUT = 180 #超时秒中Metasploit远程会话必须初始化

# Reference: http://www.cookiecentral.com/faq/#3.5
NETSCAPE_FORMAT_HEADER_COOKIES = "# Netscape HTTP Cookie File."

# Prefixes used in brute force search for web server document root 前缀用于蛮力搜索web服务器的文档根目录
...
# Suffixes used in brute force search for web server document root 后缀中使用蛮力搜索web服务器的文档根目录
...
BRUTE_DOC_ROOT_TARGET_MARK = "%TARGET%" #字符串用于标记目标名称里面使用蛮力web服务器的文档根

# Character used as a boundary in kb.chars (preferably less frequent letter)
KB_CHARS_BOUNDARY_CHAR = 'q' #字符用作边界kb。字符(最好是少信)

# CSS style used in HTML dump format CSS样式中使用HTML转储格式
